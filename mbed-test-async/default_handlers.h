...
namespace v0 {
    ...
    const struct
    {
        ...
        operator test_set_up_handler_t()    const { return test_set_up_handler_t(NULL); }
        operator test_tear_down_handler_t() const { return test_tear_down_handler_t(NULL); }

        operator case_set_up_handler_t()    const { return case_set_up_handler_t(NULL); }
        operator case_tear_down_handler_t() const { return case_tear_down_handler_t(NULL); }
     ...
     * test_set_up_handler_t handler = defaults.get_handler(default_handler);
     * // you will still need to manually check the handler before executing it
     * if (handler != ignore_handler) handler(...);
     *
     * extern test_tear_down_handler_t custom_handler(...);
     * // will return `custom_handler`
     * test_tear_down_handler_t handler = defaults.get_handler(custom_handler);
     * // you will still need to manually check the handler before executing it
     * if (handler != ignore_handler) handler(...);
     * @endcode
     */
    ...
    /// Calls `verbose_test_tear_down_handler` and then prints the greentea failure and success and end strings.
